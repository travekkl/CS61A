...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        "*** YOUR CODE HERE ***"
        look_up_frame = self
        while look_up_frame != None:
            if symbol in look_up_frame.bindings.keys():
                return look_up_frame.bindings[symbol]
            look_up_frame = look_up_frame.parent
        # END PROBLEM 3
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('number does not match')
        symbol = formals
        val = vals
        while symbol != nil:
            child_frame.define(symbol.first, val.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target] = LambdaProcedure(formals, expressions.second, env)
        return target
        # END PROBLEM 10
    else:
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    "*** YOUR CODE HERE ***"
    process = MuProcedure(formals, expressions.second)
    process.make_call_frame(process.formals, env)
    return process
    # END PROBLEM 16

...
def taxicab(a, b):
    ...
    b_street = street(b)
    b_avenue = avenue(b)
    return abs(a_street - b_street) + abs(a_avenue - b_avenue)
...
def replace_leaf(t, old, new):
    ...
    def replace_apply(a):
        if is_tree(a):
            if is_leaf(a) and a[0] == old:
                a[0] = new
            for branch in branches(a):
                replace_apply(branch)
        return a
    return replace_apply(a)
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password):
        ...
        if password not in correct_password:
            incorrect_password.append(password)
            if len(incorrect_password) <= 3:
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount, password):
        if new_password in pass_word:
            return withdraw(amount, "hax0r")
        else:
            return withdraw(amount, password)

    if old_password in pass_word:
        pass_word.append(new_password)
    else:

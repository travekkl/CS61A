...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    score_cube = score ** 3
    quotient, remainder = score_cube, 0
    sign, final_score = 1, 0
    while quotient > 0:
        quotient, remainder = quotient // 10, quotient % 10
        sign *= (-1)
        final_score += sign * remainder
    if final_score < 0:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    score0_cur, score0_pre = 0, 0
    score1_cur, score1_pre = 0, 0
    while True:
        if who == 0:
            score0_pre = score0_cur
            num_rolls0 = strategy0(score0, score1)
            score0_cur = take_turn(num_rolls0, score1, dice)       
            score0 += score0_cur
            if feral_hogs == True:
                if (score0_pre - num_rolls0) in [-2, 2]:
                    score0 += 3


        else:
            score1_pre = score1_cur
            num_rolls1 = strategy1(score1, score0)
            score1_cur = take_turn(num_rolls1, score0, dice)
            score1 += score1_cur
            if feral_hogs == True:
                if (score1_pre - num_rolls1) in [-2, 2]:
                    score1 += 3
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def announce_get_highest(score0, score1):
        if who == 0:
            cur_gain = score0 - prev_score
            cur_score = score0
        else:
            cur_gain = score1 - prev_score
            cur_score = score1
        if cur_gain > prev_high:
            high_gain = cur_gain
            print(cur_gain, "point(s)!", "That's the biggest gain yet for Player", who)
        else:
            high_gain = prev_high
        return announce_highest(who=who, prev_high=high_gain, prev_score=cur_score)
    return announce_get_highest
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    pre_average = 0
    for i in range(1, 11):
        cur_average = make_averaged(roll_dice, num_samples)(i, dice)
        if pre_average < cur_average:
            max_num = i
        pre_average = cur_average
    return max_num
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    if is_swap(bacon_score + score, opponent_score):
